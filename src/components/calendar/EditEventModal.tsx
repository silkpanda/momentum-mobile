...
export default function EditEventModal({ visible, onClose, onSuccess, event }: EditEventModalProps) {
    ...
    useEffect(() => {
        if (event && visible) {
            ...
            setStartDate(new Date(event.start?.dateTime || event.start?.date || event.startDate));
            setEndDate(new Date(event.end?.dateTime || event.end?.date || event.endDate));

            // Map attendees safely (handle IDs or Objects)
            let initialAttendees: string[] = [];
            if (event.attendees && Array.isArray(event.attendees)) {
                console.log('[EditEventModal] Raw attendees:', JSON.stringify(event.attendees));
                initialAttendees = event.attendees.map((a: any) =>
                    typeof a === 'string' ? a : (a._id || a.id || a.userId)
                ).filter(Boolean);
            }
            console.log('[EditEventModal] Initialized attendees:', initialAttendees);
    ...
    useEffect(() => {
        if (visible) {
            console.log('[EditEventModal] Available Members:', members.map(m => ({ id: m.id, userId: m.userId, name: m.firstName })));
        }
    }, [visible, members]);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await calendarService.updateGoogleEvent(event.id, eventData);

            Alert.alert('Success', 'Event updated successfully!');
            onSuccess();
            onClose();
        } catch (apiError: any) {
            console.error('Event update failed:', apiError);

            if (apiError.message?.includes('401') || apiError.message?.includes('not connected')) {
                Alert.alert('Calendar Error', 'Google Calendar not connected.');

...
export default function MorningBriefing() {
    ...
    const recentActivity = tasks
        .filter(t => (t.status === 'Completed' || t.status === 'Approved') && t.updatedAt)
                    ...
                    <View key={task._id} style={[styles.urgentCard, { backgroundColor: theme.colors.bgSurface, borderLeftColor: theme.colors.signalAlert, shadowColor: theme.colors.textPrimary }]}>
                        <View style={{ flex: 1 }}>
                            ...
                            <Text style={[styles.urgentSubtitle, { color: theme.colors.textSecondary }]}>
                                {members.find(m => task.assignedTo && task.assignedTo.includes(m.id))?.firstName} is waiting
                            </Text>
                        </View>
                        <TouchableOpacity
                            style={[styles.resolveButton, { backgroundColor: theme.colors.bgCanvas, borderColor: theme.colors.signalAlert }]}
                            onPress={() => handleQuickApprove(task._id || task.id)}
                        >
        ...
        <View style={styles.section}>
            ...
            {recentActivity.length > 0 ? (
                recentActivity.map(task => {
                    const timeString = task.updatedAt ? new Date(task.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'RECENTLY';
                    const memberName = members.find(m => task.assignedTo && task.assignedTo.includes(m.id))?.firstName || 'Someone';
